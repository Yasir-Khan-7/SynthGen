...
async def generate_synthetic_data(df: pd.DataFrame, num_rows: int) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Data stats: {st.session_state.data_stats}")
            
            return df_synthetic
            
        except pd.errors.ParserError as e:
            logger.error(f"CSV parsing error: {str(e)}")
            logger.error(f"Raw response data: {csv_data[:500]}")
            st.error(f"❌ Error parsing generated CSV data: {str(e)}")
            return pd.DataFrame()
        except Exception as e:
            logger.error(f"Unexpected error during data parsing: {str(e)}")
            logger.error(traceback.format_exc())
            st.error(f"❌ Error processing generated data: {str(e)}")
            return pd.DataFrame()
    
    except Exception as e:
        logger.error(f"Critical error in generate_synthetic_data: {str(e)}")
        logger.error(traceback.format_exc())
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            logger.error(f"CSV parsing error when loading file: {str(e)}")
...
# RESULTS VIEW
    ...
    if st.session_state.is_generating:
        ...
        if st.session_state.data_stats:
            ...
            st.warning("Data statistics unavailable. Please retry generation if metrics are needed.")
        ...
        with col2:
            try:
                ...
                logger.info(f"Excel export prepared: {len(buffer.getvalue())} bytes")
                st.download_button(
                    "⬇Download Excel",
                    buffer.getvalue(),
                    "synthetic_data.xlsx",
...
# VISUALIZATION VIEW
    ...
    if st.session_state.original_data is not None and st.session_state.generated_data is not None:
        ...
        if numeric_columns:
            ...
            logger.info(f"Column selected for visualization: {selected_column}")
            
            # Ensure selected column exists in generated data before plotting
            if selected_column not in st.session_state.generated_data.columns:
                logger.warning(f"Selected column '{selected_column}' is missing in synthetic data")
                st.warning(f"Selected column '{selected_column}' is missing in the synthetic data. Please regenerate or pick another column.")
            else:
                ...
                logger.info(f"Visualization type selected: {viz_type}")
                ...
                if viz_type == "Histogram":
                    try:
                        ...
                        with col1:
                            ...
                            """, unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)
                            logger.info("Original histogram created successfully")
                        ...
                        """, unsafe_allow_html=True)
                        st.markdown("</div>", unsafe_allow_html=True)
                        logger.info("Synthetic histogram created successfully")
                    except Exception as e:
                    ...
                    try:
                        ...
                        logger.info(f"Combined dataframe created for box plot: {combined_df.shape}")
                    ...
                    try:
                        ...
                        if len(numeric_columns) >= 2:
                            ...
                            logger.info(f"Second column selected for scatter plot: {second_column}")
                            ...
                            with col1:
                                ...
                                st.markdown("</div>", unsafe_allow_html=True)
                                logger.info("Original scatter plot created successfully")
                            ...
                            with col2:
                                ...
                                st.markdown("</div>", unsafe_allow_html=True)
                                logger.info("Synthetic scatter plot created successfully")
                ...
                try:
                    ...
                    with col1:
                        ...
                        st.markdown("</div>", unsafe_allow_html=True)
                        logger.info("Original data statistics displayed")
                    ...
                    with col2:
                        ...
                        st.markdown("</div>", unsafe_allow_html=True)
                        logger.info("Synthetic data statistics displayed")
                    logger.info("Statistical analysis section completed successfully")
                except Exception as e:
        ...
        logger.info("Visualization tab: No data available yet")
        st.markdown("""
        <div class="card">
